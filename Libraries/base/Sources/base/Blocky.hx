...
class Blocky{
	...
	public var minLight(default,null) : Int = 0;

	static var blockiesCache : Map<String,Blocky> = new Map();
	static public function get(address : String, minLight : Int = 0) : Blocky{
		var key = address;
		if(minLight > 0){
			key = key + minLight;
		}
		if(!blockiesCache.exists(key)){
			blockiesCache[key] = new Blocky(address, 6,minLight);
		}
		return blockiesCache[key];
	}

	private function new(address : String, scale : UInt = 4, minLight = 0) {
		this.scale = scale;
		this.size = 8;		
		...
		color = createColor(minLight);
		bgcolor = createColor(minLight);
		spotcolor = createColor(minLight);
	...
	function createColor(minLight : Int) : kha.Color {
		...
		//lightness can be anything from 0 to 100, but probabilities are a bell curve around 50%
		var l : Float = (((rand()+rand()+rand()+rand()) * (25 - minLight/4 )) + minLight )/ 100;
